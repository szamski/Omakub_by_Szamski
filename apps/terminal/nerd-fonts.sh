...
if ! wget -q --show-progress https://github.com/ryanoasis/nerd-fonts/releases/latest/download/CascadiaCode.zip; then
  echo "Error: Failed to download Nerd Font"
...
if ls CascadiaFont/*.ttf 1> /dev/null 2>&1; then
  ...
  echo "Done: CaskaydiaCove Nerd Font installed ($(ls CascadiaFont/*.ttf | wc -l) files)"
else
  echo "Warning: No .ttf files found in archive"
fi
...
if command -v fc-cache >/dev/null 2>&1; then
  ...
  echo "Warning: fc-cache not found, installing fontconfig..."
...
echo "Done: Nerd Fonts installation complete"
